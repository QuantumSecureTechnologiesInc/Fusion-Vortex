...
impl WasmCodeGenerator {
    ...
    fn generate_statement(&mut self, stmt: &Statement, func: &mut Function) -> Result<(), String> {
        match stmt {
            ...
            _ => {
                return Err(format!("Statement not yet supported in WASM: {:?}", stmt));
            }
        }
        Ok(())
    }
        ...
        match expr {
            ...
            Expression::BinaryOp { left, op, right } => {
                ...
                match op {
                    ...
                    _ => return Err(format!("Binary op not yet supported in WASM: {:?}", op)),
                }
            }
            ...
            _ => {
                return Err(format!("Expression not yet supported in WASM: {:?}", expr));
            }
        }
        Ok(())
    }
