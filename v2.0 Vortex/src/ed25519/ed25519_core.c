 ...
 * ed25519_core.c
 *
 * Core Ed25519 signature operations: scalar arithmetic, group operations, and
 * signing/verification.
 *
 * Algorithm Overview:
 * ───────────────────
 * EdDSA uses a modified Edwards curve over GF(p) where p = 2^255 - 19.
 * The curve equation is: -x^2 + y^2 = 1 + dx^2*y^2, where d = -121665/121666
 * mod p.
 *
 * Key Points:
...
static const uint64_t sha512_k[80] = {
    ...
    0xe9b5dba58189dabbULL, 0x3956c25bf348b538ULL, 0x59f111f1b605d019ULL,
    ...
    0x650a73548baf63deULL, 0x766a0ebb3c88b691ULL, 0x81c2c92e47edaee6ULL,
    0x92722c851482353bULL, 0xa2bfe8a14cf10364ULL, 0xa81a664bbc423001ULL,
    0xc24b8b70d0f89791ULL, 0xc76c51a30654be30ULL, 0xd192e819d6ef5218ULL,
    0xd69906245565a910ULL, 0xf40e35855771202aULL, 0x106aa07032bbd1b8ULL,
    0x19a4c116b8d2d0c8ULL, 0x1e376c081b3572c0ULL, 0x2748774cdf8eeb99ULL,
    ...
    0x431d67c49c100d0cULL, 0x4cc5d4becb3e42b6ULL, 0x597f299cfc657e2aULL,
...
static void sha512_process(uint64_t *h, const uint8_t *data) {
  ...
  for (i = 0; i < 16; i++) {
    w[i] =
        ((uint64_t)data[8 * i] << 56) | ((uint64_t)data[8 * i + 1] << 48) |
...
void sha512(uint8_t *digest, const uint8_t *message, size_t len) {
  ...
  for (i = 0; i < blocks; i++) {
    memset(block, 0, 128);
    size_t copy_len =
        (len > i * 128) ? ((len - i * 128) > 128 ? 128 : (len - i * 128)) : 0;
    if (copy_len)
      memcpy(block, message + i * 128, copy_len);
...
void sc_reduce64(uint8_t *r, const uint8_t *x) {
  ...
  for (i = 63; i >= 32; --i) {
    ...
    for (j = i - 32; j < i - 12; ++j) {
      carry +=
          h[j] + (uint64_t)h[i] * (i - j == 32 ? 8 : (i - j == 20 ? 16 : 0));
      h[j] = (uint8_t)carry;
      carry >>= 8;
    }
