 ...
 * Implements constant-time modular addition, subtraction, multiplication, and
 * inversion.
 *
...
#define LD64(x)                                                                \
  ((uint64_t)(x)[0] | ((uint64_t)(x)[1] << 8) | ((uint64_t)(x)[2] << 16) |     \
...
void fe_inv(uint32_t *h, const uint32_t *f) {
  uint32_t t[10];
  uint32_t x2[10], x4[10], x8[10], x16[10], x32[10], x64[10], x128[10];
  int i;
