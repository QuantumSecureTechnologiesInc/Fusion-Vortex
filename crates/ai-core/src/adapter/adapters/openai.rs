...
pub use self::{Message as OpenAIMessage, Usage as OpenAIUsage};
#[derive(Debug, Deserialize)]
struct StreamChunk {
    id: String,
    object: String,
...
impl OpenAIAdapter {
    ...
    pub async fn count_tokens(&self, text: &str) -> Result<usize> {
        // Approximate token count (GPT uses ~4 chars per token on average)
    ...
    pub fn calculate_cost(&self, usage: &Usage) -> f64 {
        ...
        let input_cost_total = (usage.prompt_tokens as f64 / 1000.0) * input_cost;
        let output_cost_total = (usage.completion_tokens as f64 / 1000.0) * output_cost;
...
mod tests {
    ...
    fn test_cost_calculation() {
        ...
        assert!(cost > 0.0);
        assert!(cost < 1.0); // Should be reasonable for this usage
    }
}
