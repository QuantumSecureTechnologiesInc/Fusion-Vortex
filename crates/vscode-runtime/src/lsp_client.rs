...
pub struct LspClient {
    ...
    message_id: Arc<RwLock<FI64>>,
    pending_requests: Arc<RwLock<FMap<FI64, tokio::sync::oneshot::Sender<Value>>>>,
...
impl LspClient {
    ...
    async fn initialize(&mut self, root_uri: &str) -> Result<()> {
        let params = serde_json::json!(
            { "processId" : std::process::id(), "rootUri" : root_uri, "capabilities" : {
            "textDocument" : { "completion" : { "completionItem" : { "snippetSupport" :
            true } }, "hover" : { "contentFormat" : ["markdown", "plaintext"] } } } }
        );
    ...
    async fn read_messages(
        ...
        loop {
            ...
            let content_length = headers
                ...
                .and_then(|len| len.trim().parse::<FSize>().ok());
    ...
    pub async fn completion(
        ...
        let params = serde_json::json!(
            { "textDocument" : { "uri" : uri }, "position" : { "line" : line, "character"
            : character } }
        );
    ...
    pub async fn references(
        ...
        let params = serde_json::json!(
            { "textDocument" : { "uri" : uri }, "position" : { "line" : line, "character"
            : character }, "context" : { "includeDeclaration" : true } }
        );
    ...
    pub async fn did_open(
        ...
        let params = serde_json::json!(
            { "textDocument" : { "uri" : uri, "languageId" : language_id, "version" :
            version, "text" : text } }
        );
    ...
    pub async fn did_change(
        ...
        let params = serde_json::json!(
            { "textDocument" : { "uri" : uri, "version" : version }, "contentChanges" :
            [{ "text" : text }] }
        );
