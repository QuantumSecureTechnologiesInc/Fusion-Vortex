...
impl Engine {
    ...
    pub fn navigate(&mut self, url: &str) -> Result<()> {
        ...
        let page = self
            .runtime
            .block_on(async move { browser.new_page(&url).await })?;

        // Wait for navigation
        self.runtime
            .block_on(async { page.wait_for_navigation().await })?;
    ...
    /// Click at specific coordinates (pixel-based)
    ...
    pub fn reload(&self) -> Result<()> {
        ...
        self.runtime.block_on(async move { page.reload().await })?;

        Ok(())
    }

    /// Wait for an element to appear
    pub fn wait_for_element(&self, selector: &str, timeout_ms: u64) -> Result<bool> {
        let page = self
            .current_page
            .as_ref()
            .ok_or_else(|| crate::BrowserError::Engine("No active page".to_string()))?;
        ...
        let result = self.runtime.block_on(async move {
            ...
            let wait_future = page.find_element(&selector);

            match fusion_runtime_core::timer::global_timer()
                .timeout(timeout, wait_future)
                .await
            {
                Ok(Ok(_)) => Ok(true),
                _ => Ok(false),
