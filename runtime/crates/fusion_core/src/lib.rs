...
impl fmt::Display for FusionType {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            FusionType::Tensor(tt) => write!(f, "Tensor(shape={:?}, device={})", tt.shape, tt.device),
...
mod tests {
    ...
    fn test_classical_types() {
        ...
        assert!(matches!(int_val, FusionType::Classical(ClassicalType::Int(42))));
        
        let float_val = FusionType::float(3.14);
        assert!(matches!(float_val, FusionType::Classical(ClassicalType::Float(_))));
        
        let bool_val = FusionType::bool(true);
        assert!(matches!(bool_val, FusionType::Classical(ClassicalType::Bool(true))));
    }
