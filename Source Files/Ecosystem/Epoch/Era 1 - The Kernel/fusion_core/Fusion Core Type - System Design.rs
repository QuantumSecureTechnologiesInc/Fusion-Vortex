...
Classical data (primitives, structures, collections)
Tensors (dense multi-dimensional arrays for ML/numerical computing)
Quantum circuits (quantum gates, qubits, measurements)
...
No Implicit Conversions: Classical → Tensor → Quantum require explicit casts
Quantum No-Cloning: Cannot copy quantum states (enforced by type system)
Measurement Irreversibility: Measured qubits become classical (type change)
Tensor Shape Safety: Shape mismatches caught at compile time (where possible)
Qubit Uniqueness: Each qubit can only be in one register at a time
