...
pub fn power_iteration(a: &Matrix<f64>, num_simulations: usize) -> FusionResult<(f64, Vector1D<f64>)> {
    let n = a.shape[0];
    assert_eq!(a.shape[0], a.shape[1], "Matrix must be square");

    // Random initial vector
    let mut b_k = Vector1D::new(vec![1.0; n], [n])?; // Should be random in prod
    
    // Normalize
    // b_k = b_k / norm(b_k)
    let norm = norm(&b_k);
    scale(&mut b_k, 1.0/norm);
    ...
    for _ in 0..num_simulations {
        ...
        if norm_k1 < 1e-9 { break; } // Converged to zero?
...
fn scale(v: &mut Vector1D<f64>, s: f64) {
    ...
    *v = Vector1D::new(new_data, [v.shape[0]]).unwrap();
}