...
impl DistributedTrainer {
    pub async fn new(world_size: usize, rank: usize, addresses: Vec<String>) -> StdResult<Self> {
        let mut connections = Vec::with_capacity(world_size);
        ...
        Ok(Self {
            process_group: ProcessGroup { rank, world_size, connections },
        })
    }
    ...
    pub async fn all_reduce_gradients(&self, local_grads: &mut [Variable]) -> StdResult<()> {
        ...
        println!("[DDP] Rank {} aggregating {} parameters across {} nodes.", 
                 self.process_group.rank, local_grads.len(), self.process_group.world_size);
