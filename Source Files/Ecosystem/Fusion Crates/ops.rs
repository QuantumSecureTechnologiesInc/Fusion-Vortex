...
/// Handles floating point coordinate mapping and boundary clamping safely.

use fusion_core::types::tensor::Tensor3D;
...
impl ImageOps {
    ...
    pub fn resize_bilinear(input: &Tensor3D<f64>, new_h: usize, new_w: usize) -> FusionResult<Tensor3D<f64>> {
        let (c, h, w) = (input.shape[0], input.shape[1], input.shape[2]);
        let mut output_data = Vec::with_capacity(c * new_h * new_w);

        let x_ratio = if new_w > 1 { (w - 1) as f64 / (new_w - 1) as f64 } else { 0.0 };
        let y_ratio = if new_h > 1 { (h - 1) as f64 / (new_h - 1) as f64 } else { 0.0 };
        ...
        for ch in 0..c {
            for i in 0..new_h {
                for j in 0..new_w {
                    ...
                    // Interpolate X
                    // top = A(1-w) + B(w)
                    let top = a * (1.0 - x_weight) + b * x_weight;
                    let bottom = c_val * (1.0 - x_weight) + d * x_weight;
                    
                    // Interpolate Y
                    let pixel = top * (1.0 - y_weight) + bottom * y_weight;
                    
                    output_data.push(pixel);
                }
            }
        }
