...
impl BlockQuantizedMatrix {
    ...
    pub fn quantize(matrix: &Matrix<f64>) -> FusionResult<Self> {
        ...
        // Pad if necessary (simplified: assume aligned for this impl, or error)
        if num_elements % BLOCK_SIZE != 0 {
            return Err(FusionError::InvalidDimension(
                format!("Total elements {} must be divisible by block size {}", num_elements, BLOCK_SIZE)
            ));
        }
        ...
        // Iterate over flattened data chunks
        // Note: fusion_core::Tensor uses Arc<Vec>, assuming contiguous row-major here.
        // Direct access via `get` loop for safety in Phase 2 core API limits.
        
        // Create a buffer to hold the current block
        let mut block_buffer = Vec::with_capacity(BLOCK_SIZE);

        for i in 0..num_elements {
            // Reconstruct indices from flat i
            let r = i / cols;
            let c = i % cols;
    ...
    fn quantize_block(block: &[f64]) -> (Vec<i8>, f64, f64) {
        ...
        let q_vals: Vec<i8> = block.iter().map(|&x| {
