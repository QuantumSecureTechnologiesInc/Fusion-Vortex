...
impl DeepSeekAdapter {
    pub fn new(config: DeepSeekConfig) -> Result<Self> {
        if config.api_key.is_empty() {
            anyhow::bail!("DeepSeek API key is required");
        }
        ...
        let client = Client::builder()
            ...
            .build()
            .context("Failed to create DeepSeek HTTP client")?;
    ...
    pub async fn chat_completion(
        ...
        let response = self
            ...
            .send()
            .await
            .context("DeepSeek chat completion request failed")?;
        let data: ChatCompletionResponse = response
            .json()
            .await
            .context("Invalid DeepSeek response")?;
        if data.choices.is_empty() {
            anyhow::bail!("No choices returned from DeepSeek");
        }
