...
impl QwenAdapter {
    pub fn new(config: QwenConfig) -> Result<Self> {
        if config.api_key.is_empty() {
            anyhow::bail!("Qwen API key is required");
        }
        ...
        let client = Client::builder()
            ...
            .build()
            .context("Failed to create Qwen HTTP client")?;
    ...
    pub async fn chat_completion(
        ...
        let response = self
            ...
            .send()
            .await
            .context("Qwen chat completion request failed")?;
        let data: ChatCompletionResponse = response
            .json()
            .await
            .context("Invalid Qwen response")?;
        if data.choices.is_empty() {
            anyhow::bail!("No choices returned from Qwen");
        }
