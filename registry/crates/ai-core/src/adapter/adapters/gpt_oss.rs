...
impl GptOssAdapter {
    pub fn new(config: GptOssConfig) -> Result<Self> {
        let client = Client::builder()
            ...
            .build()
            .context("Failed to create GPT-OSS HTTP client")?;
    ...
    pub async fn chat_completion(
        ...
        let response = self
            ...
            .send()
            .await
            .context("GPT-OSS chat completion request failed")?;
        let data: ChatCompletionResponse = response
            .json()
            .await
            .context("Invalid GPT-OSS response")?;
        if data.choices.is_empty() {
            anyhow::bail!("No choices returned from GPT-OSS");
        }
