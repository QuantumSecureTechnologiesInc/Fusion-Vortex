    ...
    pub fn to_kraus(&self) -> KrausOperator {
        match self {
            Self::Depolarizing(p) => {
                ...
                let e0 = Matrix::new(vec![c_1_p, Complex66::default(), Complex64::default(), c_1_p], [2, 2]).unwrap(); // Identity scalar
                ...
                KrausOperator { 
                    matrices: vec![
                        Matrix::new(vec![c_1_p, Complex64::default(), Complex64::default(), c_1_p], [2, 2]).unwrap(), // E0 (Scalar Identity)
                        Matrix::new(vec![Complex64::default(), e_base, e_base, Complex64::default()], [2, 2]).unwrap(), // E1 (X)
                        // ... Y and Z operators ...
                    ]
                }
            },
            Self::AmplitudeDamping(gamma) => {
                ...
                let e0_data = vec![s_1_gamma, Complex64::default(), Complex64::default(), s_1_gamma];
                let e0 = Matrix::new(e0_data, [2, 2]).unwrap();
                
                // E1 (Photon emitted, state |1> -> |0>)
                let e1_data = vec![Complex64::default(), s_gamma, Complex64::default(), Complex64::default()];
                let e1 = Matrix::new(e1_data, [2, 2]).unwrap();
                
                KrausOperator { matrices: vec![e0, e1] }
            },
            Self::PhaseDamping(p) => {
                ...
                // E1 = sqrt(p) * Z
                let e1_data = vec![s_p, Complex64::default(), Complex64::default(), s_p * Complex64::new(-1.0, 0.0)];
                let e1 = Matrix::new(e1_data, [2, 2]).unwrap();
