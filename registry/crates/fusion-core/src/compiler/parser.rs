...
impl Parser {
    ...
    #[allow(dead_code)]
    fn expect_peek(&mut self, token: Token) -> FBool {
        if std::mem::discriminant(&self.peek_token.token)
    ...
    pub fn parse_program(&mut self) -> Result<Program, FString> {
        ...
        while self.current_token.token != Token::EOF {
            match self.current_token.token {
                ...
                Token::Extern => {
                    declarations.push(Declaration::Extern(self.parse_extern()?))
                }
                Token::Struct => {
                ...
                _ => {
                    return Err(
                        format!(
                            "Unexpected token {:?} at start of declaration", self
                            .current_token.token
                        ),
                    );
                }
    ...
    fn parse_type(&mut self) -> Result<Type, FString> {
        match &self.current_token.token {
            Token::Identifier(s) => {
                match s.as_str() {
                    "Int" => Ok(Type::Int),
                    "Bool" => Ok(Type::Bool),
                    "String" => Ok(Type::FString),
                    "Void" => Ok(Type::Void),
    ...
    fn parse_function(&mut self) -> Result<FunctionDecl, FString> {
        self.advance_token();
        ...
        if self.current_token.token != Token::LBrace {
            return Err(
                format!(
                    "Expected {{ to start function body, got {:?}", self.current_token
                    .token
                ),
            );
        }
    ...
    fn parse_extern(&mut self) -> Result<ExternDecl, FString> {
        self.advance_token();
    ...
    fn parse_struct(&mut self) -> Result<StructDecl, FString> {
        self.advance_token();
        let name = match &self.current_token.token {
            Token::Identifier(s) => s.clone(),
    ...
    fn parse_params(&mut self) -> Result<FVec<(FString, Type)>, FString> {
        self.advance_token();
    ...
    fn parse_block(&mut self) -> Result<FVec<Statement>, FString> {
        self.advance_token();
    ...
    fn parse_statement(&mut self) -> Result<Statement, FString> {
        match self.current_token.token {
            Token::Let => {
                self.advance_token();
                let name = match &self.current_token.token {
    ...
    fn parse_expression(&mut self) -> Result<Expression, FString> {
    ...
    fn parse_binary(&mut self, min_precedence: u8) -> Result<Expression, FString> {
    ...
    fn get_precedence(&self, op: &BinaryOp) -> u8 {
        match op {
            ...
            | BinaryOp::NotEqual
            | BinaryOp::LessThan
            | BinaryOp::GreaterThan => 1,
            BinaryOp::Add | BinaryOp::Sub => 2,
            BinaryOp::Mul | BinaryOp::Div => 3,
        }
    }
    fn parse_primary(&mut self) -> Result<Expression, FString> {
        match &self.current_token.token {
            Token::Identifier(s) => {
                let name = s.clone();
                self.advance_token();
            ...
            Token::StringLiteral(s) => {
                let val = s.clone();
                self.advance_token();
                Ok(Expression::Literal(Literal::FString(val)))
            }
            Token::True => {
                self.advance_token();
            ...
            _ => {
                Err(
                    format!(
                        "Unexpected token in expression: {:?}", self.current_token.token
                    ),
                )
            }
        }
