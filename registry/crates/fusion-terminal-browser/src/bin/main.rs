...
fn init_logging(verbose: bool) {
    let filter = if verbose {
        EnvFilter::new("fusion_terminal_browser=debug,chromiumoxide=info")
    } else {
        EnvFilter::new("fusion_terminal_browser=info,chromiumoxide=warn")
    };
    ...
    match cli.command {
        ...
        Some(Commands::Exec { url, script }) => {
            ...
            let json_str = serde_json::to_string_pretty(&result)
                .map_err(|e| fusion_terminal_browser::BrowserError::Generic(e.to_string()))?;
            println!("{}", json_str);
        }
