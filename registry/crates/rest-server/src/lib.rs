...
type Handler = fn(Request<Vec<u8>>) -> StdResult<Response<Vec<u8>>>;
...
impl RestServer {
    ...
    pub fn handle_request(&self, request: Request<Vec<u8>>) -> StdResult<Response<Vec<u8>>> {
...
mod tests {
    ...
    fn dummy_handler(_req: Request<Vec<u8>>) -> StdResult<Response<Vec<u8>>> {
        Ok(Response::new(vec![]))
    }
