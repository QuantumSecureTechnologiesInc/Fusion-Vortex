...
impl PagedAttentionManager {
    ...
    pub fn new(num_blocks: usize, num_heads: usize, head_dim: usize) -> Self {
        ...
        for i in 0..num_blocks {
            physical_blocks.push(Arc::new(RwLock::new(KVCacheBlock::new(
                num_heads, head_dim,
            ))));
            free_blocks.push(i);
        }
    ...
    pub fn allocate_block(&mut self, seq_id: u64) -> FusionResult<()> {
        if let Some(block_idx) = self.free_blocks.pop() {
            ...
            let mut writer = block
                .write()
                .map_err(|_| fusion_core::FusionError::CompilationError("Lock poisoned".into()))?;
            writer.num_filled = 0;

            Ok(())
        } else {
            Err(fusion_core::FusionError::CompilationError(
    ...
    pub fn append_token(
        ...
        let table =
            self.block_tables
                .get_mut(&seq_id)
                .ok_or(fusion_core::FusionError::UnknownVariable(format!(
                    "Seq {}",
                    seq_id
                )))?;
