//! # Fiber/Coroutine API
//!
//! Low-latency task switching mechanism that allows the scheduler to switch
//! tasks faster than OS threads. This is the 'F' in Fusion's future-based concurrency.
//!
//! ## Architecture
//!
...
pub enum FiberState {
    ...
    Running,
    /// Fiber is suspended (waiting for event)
    Suspended,
    /// Fiber has completed
    Completed,
}
...
impl FiberScheduler {
    ...
    /// # Arguments
    ///
    /// * `stack_size` - Stack size in bytes (default: 64KB)
    /// * `priority` - Fiber priority (0-255)
    ///
    /// # Returns
    ///
    /// Fiber ID
