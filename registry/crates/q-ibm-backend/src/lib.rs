...
pub struct IBMResults {
    pub counts: FMap<FString, FU32>,
    pub success: FBool,
}
#[allow(dead_code)]
pub struct IBMQuantumBackend {
    api_token: FString,
    backend_name: FString,
    endpoint: FString,
}
impl IBMQuantumBackend {
    pub fn new(api_token: FString, backend_name: FString) -> Self {
        Self {
            api_token,
            backend_name,
    ...
    fn circuit_to_qobj(&self, circuit: &QuantumCircuit) -> FString {
        let mut qobj = serde_json::json!(
            { "qobj_id" : uuid::Uuid::new_v4().to_string(), "type" : "QASM",
            "schema_version" : "1.3.0", "experiments" : [{ "instructions" : [], "header"
            : { "n_qubits" : circuit.num_qubits, "memory_slots" : circuit.num_qubits, }
            }], "header" : { "backend_name" : self.backend_name, }, "config" : { "shots"
            : 1024, "memory" : true, } }
        );
        let mut instructions = Vec::new();
        for (gate, qubits) in &circuit.operations {
            let instruction = match gate.name.as_str() {
                "H" => serde_json::json!({ "name" : "h", "qubits" : qubits, }),
                "X" => serde_json::json!({ "name" : "x", "qubits" : qubits, }),
                "CNOT" => serde_json::json!({ "name" : "cx", "qubits" : qubits, }),
                "RZ" => {
                    let angle = 0.0;
                    serde_json::json!(
                        { "name" : "rz", "qubits" : qubits, "params" : [angle], }
                    )
                }
                _ => {
                    serde_json::json!(
                        { "name" : gate.name.to_lowercase(), "qubits" : qubits, }
                    )
                }
            };
        ...
        for i in 0..circuit.num_qubits {
            instructions
                .push(
                    serde_json::json!(
                        { "name" : "measure", "qubits" : [i], "memory" : [i], }
                    ),
                );
        }
    ...
    pub async fn get_backend_info(&self) -> StdResult<FMap<FString, serde_json::Value>> {
        let mut info = HashMap::new();
        info.insert("backend_name".to_string(), serde_json::json!(self.backend_name));
    ...
    pub async fn list_backends(&self) -> StdResult<FVec<FString>> {
        Ok(
            vec![
                "ibmq_qasm_simulator".to_string(), "ibm_brisbane".to_string(),
                "ibm_kyoto".to_string(),
            ],
        )
    }
